...
class IPDoubleStreamBlockProcessor(nn.Module):
    ...
    def __init__(self, context_dim, hidden_dim):
        ...
        nn.init.xavier_uniform_(self.ip_adapter_double_stream_k_proj.weight)
        nn.init.zeros_(self.ip_adapter_double_stream_k_proj.bias)

        nn.init.xavier_uniform_(self.ip_adapter_double_stream_v_proj.weight)
...
class DoubleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if image_proj is None:
            ...
            return self.processor(self, img, txt, vec, pe, image_proj=image_proj, ip_scale=ip_scale)
...
class IPSingleStreamBlockProcessor(nn.Module):
    ...
    def __init__(self, context_dim, hidden_dim):
        ...
        nn.init.xavier_uniform_(self.ip_adapter_single_stream_k_proj.weight)
        nn.init.xavier_uniform_(self.ip_adapter_single_stream_v_proj.weight)
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(
        ...
        if image_proj is None:
            ...
            return self.processor(self, x, vec, pe, image_proj=image_proj, ip_scale=ip_scale)
...
# --- LoRA + IP-Adapter 결합 프로세서 ---

class IPDoubleStreamBlockLoraProcessor(nn.Module):
    """
    DoubleStreamBlockLoraProcessor와 IPDoubleStreamBlockProcessor의 로직을 결합
    """
    def __init__(self, dim: int, rank=4, network_alpha=None, lora_weight=1, dtype=None, context_dim=4096, hidden_dim=3072):
        super().__init__()
        # 1. LoRA 모듈 (from DoubleStreamBlockLoraProcessor)
        self.qkv_lora1 = LoRALinearLayer(dim, dim * 3, rank, network_alpha, dtype=dtype)
        self.proj_lora1 = LoRALinearLayer(dim, dim, rank, network_alpha, dtype=dtype)
        self.qkv_lora2 = LoRALinearLayer(dim, dim * 3, rank, network_alpha, dtype=dtype)
        self.proj_lora2 = LoRALinearLayer(dim, dim, rank, network_alpha, dtype=dtype)
        ...
        # 2. IP-Adapter 모듈 (from IPDoubleStreamBlockProcessor)
        self.ip_adapter_double_stream_k_proj = nn.Linear(context_dim, hidden_dim, bias=True)
        self.ip_adapter_double_stream_v_proj = nn.Linear(context_dim, hidden_dim, bias=True)
        nn.init.xavier_uniform_(self.ip_adapter_double_stream_k_proj.weight)
        nn.init.zeros_(self.ip_adapter_double_stream_k_proj.bias)
    ...
    def forward(self, attn, img, txt, vec, pe, image_proj=None, ip_scale=1.0, **attention_kwargs):
        # --- 1. LoRA 로직 적용 (DoubleStreamBlockLoraProcessor.forward) ---
        img_mod1, img_mod2 = attn.img_mod(vec)
        txt_mod1, txt_mod2 = attn.txt_mod(vec)

        img_modulated = attn.img_norm1(img)
        img_modulated = (1 + img_mod1.scale) * img_modulated + img_mod1.shift
        img_qkv = attn.img_attn.qkv(img_modulated) + self.qkv_lora1(img_modulated) * self.lora_weight # LoRA
        img_q, img_k, img_v = rearrange(img_qkv, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads)
        img_q, img_k = attn.img_attn.norm(img_q, img_k, img_v)

        txt_modulated = attn.txt_norm1(txt)
        txt_modulated = (1 + txt_mod1.scale) * txt_modulated + txt_mod1.shift
        txt_qkv = attn.txt_attn.qkv(txt_modulated) + self.qkv_lora2(txt_modulated) * self.lora_weight # LoRA
        txt_q, txt_k, txt_v = rearrange(txt_qkv, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads)
        txt_q, txt_k = attn.txt_attn.norm(txt_q, txt_k, txt_v)
        ...
        img = img + img_mod1.gate * attn.img_attn.proj(img_attn) + img_mod1.gate * self.proj_lora1(img_attn) * self.lora_weight # LoRA
        img = img + img_mod2.gate * attn.img_mlp((1 + img_mod2.scale) * attn.img_norm2(img) + img_mod2.shift)

        txt = txt + txt_mod1.gate * attn.txt_attn.proj(txt_attn) + txt_mod1.gate * self.proj_lora2(txt_attn) * self.lora_weight # LoRA
        txt = txt + txt_mod2.gate * attn.txt_mlp((1 + txt_mod2.scale) * attn.txt_norm2(txt) + txt_mod2.shift)
        
        # --- 2. IP-Adapter 로직 적용 (IPDoubleStreamBlockProcessor.forward) ---
...
class IPSingleStreamBlockLoraProcessor(nn.Module):
    """
    SingleStreamBlockLoraProcessor와 IPSingleStreamBlockProcessor의 로직을 결합
    """
    def __init__(self, dim: int, rank: int = 4, network_alpha = None, lora_weight: float = 1, dtype=None, context_dim=4096, hidden_dim=3072, mlp_ratio: float = 4.0):
        super().__init__()
        # 1. LoRA 모듈 (from SingleStreamBlockLoraProcessor)
        mlp_hidden_dim = int(dim * mlp_ratio)
        proj_lora_in_dim = dim + mlp_hidden_dim 
        self.qkv_lora = LoRALinearLayer(dim, dim * 3, rank, network_alpha, dtype=dtype)
        self.proj_lora = LoRALinearLayer(proj_lora_in_dim, dim, rank, network_alpha, dtype=dtype) # 하드코딩된 15360 대신 계산
        self.lora_weight = lora_weight
        
        # 2. IP-Adapter 모듈 (from IPSingleStreamBlockProcessor)
        self.ip_adapter_single_stream_k_proj = nn.Linear(context_dim, hidden_dim, bias=False)
        self.ip_adapter_single_stream_v_proj = nn.Linear(context_dim, hidden_dim, bias=False)
        nn.init.xavier_uniform_(self.ip_adapter_single_stream_k_proj.weight)
        nn.init.xavier_uniform_(self.ip_adapter_single_stream_v_proj.weight)

    def forward(self, attn: nn.Module, x: Tensor, vec: Tensor, pe: Tensor, image_proj: Tensor | None = None, ip_scale: float = 1.0) -> Tensor:
        
        # --- 1. LoRA 로직 적용 (SingleStreamBlockLoraProcessor.forward) ---
        mod, _ = attn.modulation(vec)
        x_mod = (1 + mod.scale) * attn.pre_norm(x) + mod.shift
        qkv, mlp = torch.split(attn.linear1(x_mod), [3 * attn.hidden_size, attn.mlp_hidden_dim], dim=-1)
        qkv = qkv + self.qkv_lora(x_mod) * self.lora_weight # LoRA
        ...
        # --- 2. IP-Adapter 로직 적용 (IPSingleStreamBlockProcessor.forward) ---
        attn_out = attn_1
        if image_proj is not None:
            ip_query = q
            ip_key = self.ip_adapter_single_stream_k_proj(image_proj)
            ...
            attn_out = attn_1 + ip_scale * ip_attention # IP 어텐션 결과 결합

        # --- 3. 최종 출력 (LoRA 로직) ---
        output_cat = torch.cat((attn_out, attn.mlp_act(mlp)), 2)
        output = attn.linear2(output_cat)
        output = output + self.proj_lora(output_cat) * self.lora_weight # LoRA
        output = x + mod.gate * output
        return output