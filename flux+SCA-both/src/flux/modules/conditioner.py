...
class HFEmbedder(nn.Module):
    def __init__(self, version: str, max_length: int, **hf_kwargs):
        ...
        if version == "/tiamat-NAS/shiwenda/models/FLUX.1-dev/text_encoder_2":
            self.is_clip= False
        else:
            self.is_clip= True
        ...
        if self.is_clip:
            self.tokenizer: CLIPTokenizer = CLIPTokenizer.from_pretrained("/tiamat-NAS/shiwenda/models/FLUX.1-dev/tokenizer", max_length=max_length)
            self.hf_module: CLIPTextModel = CLIPTextModel.from_pretrained("/tiamat-NAS/shiwenda/models/FLUX.1-dev/text_encoder", **hf_kwargs)
        else:
            self.tokenizer: T5Tokenizer = T5Tokenizer.from_pretrained('/tiamat-NAS/shiwenda/models/FLUX.1-dev/tokenizer_2', max_length=max_length)
            self.hf_module: T5EncoderModel = T5EncoderModel.from_pretrained('/tiamat-NAS/shiwenda/models/FLUX.1-dev/text_encoder_2', **hf_kwargs)
