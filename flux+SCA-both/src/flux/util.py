...
# S-TODO 读 new_dit
def load_new_flow_model(path: str, device: str | torch.device = "cuda", hf_download: bool = True):
    # Loading Flux
    print("Init model")
    ckpt_path = path
    ...
    if ckpt_path is not None:
        ...
        # sd = load_sft(ckpt_path, device=str(device)) # ckpt_path 之前是 .safetensor，new_dit 是 .bin
        # 加载 checkpoint 文件
...
def load_image_encoder(device: str | torch.device = "cuda"):
    return CLIPVisionModelWithProjection.from_pretrained("openai/clip-vit-large-patch14").eval().requires_grad_(False).to(
