...
class TCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt_plain = item['caption'] # 수ㅜ정
        prompt_html = item['caption'] # 수ㅜ정
...
def set_trainable_text_parts(t5_model, text_proj, etc_tokens: list):
    """
    (수정됨) Text Projection만 활성화하고 ETC 토큰은 동결(FROZEN) 상태로 둡니다.
    """
    text_proj.requires_grad_(True)
    print("✅ Text Projection (768→4096): Unfrozen")
    ...
    if num_new_tokens > 0:
        # (수정) requires_grad=True 및 훅(hook) 등록 로직 제거
        print(f"✅ mT5: {num_new_tokens} ETC-Tokens are FROZEN (as requested).")
...
class Trainer:
    ...
    def setup_models(self):
        ...
        self.t5_model, self.t5_tokenizer = load_mt5_with_etc_tokens(
            self.cfg_model['mt5_model_name'], self.cfg_model['etc_tokens'], device=self.device
        )
        self.siglip_model, self.siglip_tokenizer = load_siglip(
            self.cfg_model['siglip_model_name'], device=self.device
        )
    ...
    def setup_data(self):
        ...
        self.train_dataloader = DataLoader(
            train_dataset, 
            batch_size=self.cfg_train['batch_size'], # per-GPU, cfg_train[’batch_size’]×num_gpus×gradient_accumulation_steps
            shuffle=True, 
    ...
    def training_step(self, batch):
        ...
        # (수정) 전체 mT5 인코딩 과정을 no_grad()로 감쌉니다
        with torch.no_grad():
            # (수정) 임베딩 호출을 no_grad() 안으로 이동
    ...
    def train(self):
        ...
        if self.resume_from:
            if Path(self.resume_from).exists():
                ...
                self.epoch = 0
        # -----------------------------
        initial_step = self.resume_step_in_epoch
    ...
    def load_checkpoint(self, checkpoint_dir: str):
        ...
        try:
            ...
            if lora_path.exists():
                ...
                unwrapped_flux.load_state_dict(load_sft(lora_path, device="cpu"), strict=False) 
                self.accelerator.print("✅ Loaded LoRA weights.")
            
            if proj_path.exists():
                unwrapped_proj = self.accelerator.unwrap_model(self.text_proj)
                unwrapped_proj.load_state_dict(load_sft(proj_path, device="cpu"), strict=False)
            ...
            if embed_path.exists():
                ...
                unwrapped_t5_embed.load_state_dict(load_sft(embed_path, device="cpu"), strict=False)
        ...
        if opt_path.exists():
            ...
            self.accelerator.print("✅ Skipping optimizer state load (parameter groups changed). Resuming with fresh optimizer.")
        else:
        ...
        if sch_path.exists():
            ...
            self.accelerator.print("✅ Skipping scheduler state load (resuming with fresh scheduler).")
        else:
        ...
        if progress_path.exists():
            ...
            if steps_per_epoch == 0:
                raise ValueError("Dataloader is empty, cannot calculate steps_per_epoch.")
