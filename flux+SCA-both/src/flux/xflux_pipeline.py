    ...
    def set_ip(self, local_path: str = None, repo_id = None, name: str = None):
        ...
        for name, attn_processor in self.model.attn_processors.items():
            ...
            if match:
                ...
                if name.startswith("double_blocks") and layer_index%double_internal==0:
                    ip_attn_procs[name] = IPDoubleStreamBlockProcessor(4096, 3072)
                    ip_attn_procs[name].to(self.device, dtype=torch.bfloat16)
                elif name.startswith("single_blocks") and layer_index%single_internal==0:
        ...
        adapter_modules = torch.nn.ModuleList([ip_module for ip_module in self.model.attn_processors.values() if 'IP' in str(ip_module)])
        if ".safetensors" in local_path:
            ip_adapter_state_dict = {k.replace("ip_adapter.", ""): v for k, v in checkpoint.items() if k.startswith("ip_adapter.")}
        else:
            ip_adapter_state_dict = checkpoint['ip_adapter']
        print(adapter_modules.load_state_dict(ip_adapter_state_dict,strict=True))
        ...
        if ".safetensors" in local_path:
            image_proj_state_dict = {k.replace("image_proj.", ""): v for k, v in checkpoint.items() if k.startswith("image_proj.")}
        else:
            image_proj_state_dict = checkpoint['image_proj']
        print(self.improj.load_state_dict(image_proj_state_dict,strict=True))
